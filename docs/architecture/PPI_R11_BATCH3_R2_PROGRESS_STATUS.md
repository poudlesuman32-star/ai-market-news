...
Fresh read-only prerequisite/control-plane observations on exact `ai-market-news` main `41976b8cd5f7f0758c2d5425fd2c7e6522634e1a` now include public-first chain-depth bridge run `34710895723`, attempt `1`, schedule event, completed `failure` at `2026-09-12T18:20:44Z`, followed by immutable snapshot review bridge run `34711975650`, attempt `1`, schedule event, completed `success` at `2026-09-12T18:41:51Z`. The failed chain-depth bridge did not regress the four retained prerequisites: its bridge job `103599381225` failed while re-verifying the older OpenFIGI review because exact non-expired artifact resolution for review run `33262596949-1` returned no unique artifact. Stable-ID allocation/review and immutable snapshot/review artifacts above remain independently retained and unexpired. These bridge observations create no pilot evidence and no registry credit.
...
Newer repository-level public-first observations supersede the issue body only for freshness: chain-depth bridge `34710895723-1` failed on historical OpenFIGI review artifact lifetime, while immutable snapshot review bridge `34711975650-1` succeeded on exact `ai-market-news` main `41976b8cd5f7f0758c2d5425fd2c7e6522634e1a`. Neither changes R11 pilot evidence, registry credit, or downstream authority.

Newest producer remediation-controller run is `34714020224`, attempt `1`, schedule event, exact producer main `2bbef4dc81c65ab2ee2b723f1bd4de5e34a90e88`, completed `success` at `2026-09-12T19:23:26Z`. Reconcile job `103607780647` completed `success` and reported that `codex/ppi-r11-r2-provenance-attestation` is not ahead of `main`; merge `false`, acquisition `false`, publication `false`. This dependency-safe no-op does not change R11 evidence or credit.

A newer `ai-market-news` migration-autopilot observation, run `34714165276`, attempt `1`, schedule event, completed `success` on exact main `41976b8cd5f7f0758c2d5425fd2c7e6522634e1a` at `2026-09-12T19:26:35Z`. It is control-plane-only and does not create accepted R11 pilot evidence or registry credit.
